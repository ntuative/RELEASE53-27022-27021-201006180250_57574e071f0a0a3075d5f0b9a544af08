package com.sulake.habbo.avatar.actions
{
   ...
   public class AvatarActionManager
   {
      ...
      public function AvatarActionManager(param1:IAssetLibrary, param2:String)
      {
         ...
         switch(param2)
         {
            case AvatarType.const_78:
               if(!var_185.hasAsset("HabboAvatarActions"))
               {
                  Logger.log("[AvatarStructure] Could not init human Action Data.");
               }
               else
               {
                  _loc3_ = var_185.getAssetByName("HabboAvatarActions").content as XML;
               }
               break;
            case AvatarType.PET:
               if(!var_185.hasAsset("HabboAvatarPetActions"))
               {
                  Logger.log("[AvatarStructure] Could not init pet Action Data.");
               }
               else
               {
                  _loc3_ = var_185.getAssetByName("HabboAvatarPetActions").content as XML;
               }
         }
         if(_loc3_ == null)
         {
      ...
      private function parseActionOffsets() : void
      {
         ...
         {
            _loc3_ = _loc2_.state;
            if(var_185.hasAsset("action_offset_" + _loc3_))
            {
               _loc4_ = var_185.getAssetByName("action_offset_" + _loc3_).content as XML;
               if(_loc4_.@avatartype == var_671)
               {
