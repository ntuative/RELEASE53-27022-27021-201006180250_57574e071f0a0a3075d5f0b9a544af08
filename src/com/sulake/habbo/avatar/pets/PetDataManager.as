package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      private var var_130:AvatarRenderManager;
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(var_442.getValue(_loc4_) == null)
               {
                  var_442.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(var_1084.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(var_1085.indexOf(_loc6_) == -1)
               {
                  ...
                  var_258.push(_loc8_);
                  var_1085.push(_loc6_);
                  if(param3 != null && var_703.indexOf(param3) == -1)
                  {
                     var_703.push(param3);
                  }
               }
      ...
      private function onLoaderComplete(param1:Event) : void
      {
         ...
         var_1084.push(_loc2_.url);
         var _loc3_:int = var_258.indexOf(_loc2_);
         if(_loc3_ >= 0)
         {
      ...
      {
         var _loc3_:IPetData = var_442.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
            assetsReady(param1,AvatarScaleType.const_54,param2);
         }
         return _loc3_;
      }
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_703.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      {
         var _loc4_:PetData = var_442.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_442.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_442.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.OpenedToWebPopup.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
