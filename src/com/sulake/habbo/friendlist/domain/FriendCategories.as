package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_433:IFriendCategoriesDeps;
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_468).received = true;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_433.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_433.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_433.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_433.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_433.view.setNewMessageArrived();
               var_433.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_468);
         var _loc3_:FriendCategory = findCategory(_loc2_);
         if(_loc3_ != null)
         {
            _loc3_.addFriend(param1);
            var_794[param1.id] = param1;
            return _loc3_;
         }
