package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_439 >= var_316.length)
         {
            return "";
         }
         var petColor:IPetColor = var_316[var_439] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_624[var_1580] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_439 = param1.index;
         if(var_439 < 0 || var_439 > var_316.length)
         {
            var_439 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_587.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_116,onTextWindowEvent);
         var_587 = new Map();
         ...
         var_894 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_316 = [];
         var_624 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_894,this);
               if(_loc11_)
               {
                  var_624 = _loc11_.breeds;
                  var_316 = _loc11_.colors;
                  if(var_316 == null)
                  {
                     var_316 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_624.length)
         {
            _loc12_ = var_624[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_894,_loc12_.id),getRaceLocalizationKey(var_894,_loc12_.id)));
            _loc6_++;
         }
         var_587.add(var_894,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_989,onApproveNameResult);
         _loc5_ = var_587.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_316)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_587.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_439 >= 0 && var_439 < var_316.length)
         {
            _loc7_ = var_316[var_439] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_624[var_1580] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_894,_loc3_.id,_loc2_,AvatarScaleType.const_54,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_293,AvatarAction.const_709);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_35,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_35),true);
         _loc4_.dispose();
