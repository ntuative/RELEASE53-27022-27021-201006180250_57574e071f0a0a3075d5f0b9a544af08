package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener
   {
      ...
      private var var_272:BitmapData = null;
      
      private var var_232:int = 0;
      
      private var var_729:int = 0;
      
      private var var_1374:String = "1.1";
      ...
      override public function init() : void
      {
         ...
         if(_loc1_ != null)
         {
            ...
            for each(_loc2_ in page.offers)
            {
               ...
               {
                  case "floor":
                     for each(_loc9_ in var_143.floors.pattern)
                     {
                        _activeFloorPatterns.push(String(_loc9_.@id));
                        var_1283.push(_loc2_);
                     }
                     break;
                  case "wallpaper":
                     for each(_loc10_ in var_143.walls.pattern)
                     {
                     ...
                     break;
                  case "landscape":
                     for each(_loc11_ in var_143.landscapes.pattern)
                     {
      ...
      public function imageReady(param1:int, param2:BitmapData) : void
      {
         ...
         if(var_272 != null && var_271 != null)
         {
            setRoomImage(var_272,var_271);
         }
      }
      ...
      private function updateConfiguration() : void
      {
         ...
         for each(_loc1_ in var_143.walls.pattern)
         {
            if(_loc1_.@id == _loc2_)
            {
               if(var_232 >= 0)
         ...
         for each(_loc1_ in var_143.floors.pattern)
         {
            if(_loc1_.@id == _loc5_)
            {
               _loc11_ = _loc1_.children()[var_269];
               if(_loc11_ != null)
               {
         ...
         for each(_loc1_ in var_143.landscapes.pattern)
         {
            if(_loc1_.@id == _loc7_)
            {
      ...
      private function changePattern(param1:String, param2:int) : void
      {
         ...
         switch(param1)
         {
            case const_195:
               ...
               var_731 = 0;
               _loc5_ = "null";
               for each(_loc8_ in var_143.walls.pattern)
               {
                  if(_loc8_.@id == _loc5_)
                  {
            ...
            case const_194:
               ...
               for each(_loc9_ in var_143.floors.pattern)
               {
                  if(_loc9_.@id == _loc6_)
                  {
                     var_730 = _loc9_.children().length();
                  }
               }
            ...
            case const_90:
               ...
               var_729 = 0;
               _loc7_ = "null";
               for each(_loc10_ in var_143.landscapes.pattern)
               {
                  if(_loc10_.@id == _loc7_)
                  {
                     var_729 = _loc10_.children().length();
                  }
               }
      ...
      private function onClick(param1:WindowMouseEvent) : void
      {
         ...
         switch(_loc2_.name)
         {
            ...
            case "ctlg_buy_wall":
               ...
               if(_loc3_ != null)
               {
                  (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(_loc3_,page,var_1375);
               }
               break;
            ...
            case "ctlg_landscape_pattern_next":
               changePattern(const_90,-1);
               break;
            case "ctlg_landscape_pattern_prev":
               changePattern(const_90,1);
               break;
            case "ctlg_landscape_color_prev":
               changeColor(const_90,-1);
               break;
            case "ctlg_landscape_color_next":
               changeColor(const_90,1);
               break;
            case "ctlg_buy_landscape":
               _loc3_ = var_1282[var_314];
               if(_loc3_ != null)
               {
                  (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(_loc3_,page,var_1374);
               }
               break;
            default:
      ...
      private function retrieveSpacesPreview() : void
      {
         var _loc4_:* = null;
         var _loc5_:* = null;
         var _loc2_:ImageResult = page.viewer.roomEngine.getRoomImage(_floorType,var_1375,var_1374,64,this,"window_romantic_wide");
