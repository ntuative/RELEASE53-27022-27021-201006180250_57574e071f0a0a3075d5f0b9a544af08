package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
         ...
         if(param1.bytesAvailable >= 4)
         {
            ...
            switch(_loc2_)
            {
               case 67324752:
                  ...
                  var_510 = new FZipFile(this,charEncoding);
                  break;
               case 33639248:
               case 101010256:
                  var_96 = parseIdle;
                  break;
               default:
                  dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_115,"Unknown record signature."));
            }
            return true;
         }
         return false;
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_115))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_115,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_97 && var_96 == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_115,"EOF"));
            }
         }
      }
      ...
      {
         if(var_510.parse(param1))
         {
            var_48.push(var_510);
            if(false)
            {
               var_131["null"] = var_510;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1385,var_510));
            var_510 = null;
            if(var_96 != parseIdle)
            {
               var_96 = parseSignature;
